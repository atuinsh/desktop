...
mod tests {
    ...
    async fn test_script_chaining_with_output_variables() {
        ...
        // Verify output was stored
        {
            ...
            let runbook_vars = stored.get(&runbook_id.to_string()).expect("Runbook should have variables");
            assert_eq!(runbook_vars.get("greeting").expect("greeting should be stored"), "Hello from script 1");
        }
        ...
        // Verify both outputs are stored
        {
            ...
            let runbook_vars = stored.get(&runbook_id.to_string()).expect("Runbook should have variables");
            assert_eq!(runbook_vars.get("greeting").expect("greeting should be stored"), "Hello from script 1");
            assert_eq!(runbook_vars.get("combined").expect("combined should be stored"), "Previous greeting was: Hello from script 1");
        }
    }
