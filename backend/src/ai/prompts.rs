...
impl AIPrompts {
    pub fn system_prompt(block_summary: &str) -> String {
        indoc! {"
            ...
            Before operating on these blocks, debugging behavior of these blocks, or inserting new instances of these blocks,
            ...
            Prefer generating or updating blocks in SMALL, related batches rather than all at once, as doing so causes the user's UI
            to hang while you generate the tool usage information.

            ## CUSTOM BLOCK SUMMARY:
            {block_summary}

            Note that the SQL-based blocks - SQLite, Postgres, MySQL, and ClickHouse - use the exact same prop structure.
            ...
            - Script blocks can save output to template variables via its 'outputVariable' prop, while Terminal blocks cannot
            ...
            - Dropdown blocks do NOT pause serial execution - they use whatever value is currently selected
