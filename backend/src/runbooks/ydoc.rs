...
#[tauri::command]
pub async fn save_ydoc_for_runbook<R: Runtime>(app: AppHandle<R>, request: Request<'_>) -> Result<(), String> {
...
#[tauri::command]
pub async fn load_ydoc_for_runbook<R: Runtime>(app: AppHandle<R>, runbook_id: &str) -> Result<Response, String> {
    let state = app.state::<crate::state::AtuinState>();
    let db = state
        .db_instances
        .get_pool("runbooks")
