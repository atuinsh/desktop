...
use crate::runtime::blocks::document::{BlockContext, DocumentContext};
...
impl Block {
    ...
    pub async fn passive_context(
        ...
        match self {
            Block::LocalVar(local_var) => local_var.passive_context().await,
            Block::Var(var) => var.passive_context().await,
            Block::Environment(environment) => environment.passive_context().await,
            Block::Directory(directory) => directory.passive_context().await,
            Block::SshConnect(ssh_connect) => ssh_connect.passive_context().await,
            Block::Host(host) => host.passive_context().await,
