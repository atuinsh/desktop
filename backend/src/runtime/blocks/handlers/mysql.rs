...
mod tests {
    ...
    fn create_test_mysql(query: &str) -> Mysql {
        Mysql::builder()
            .id(Uuid::new_v4())
            .name("Test MySQL")
            .query(query)
    ...
    fn test_handler_block_type() {
        let handler = MySQLHandler;
        assert_eq!(handler.block_type(), "mysql");
    }

    #[test]
    fn test_no_output_variable() {
        let handler = MySQLHandler;
        let mysql = create_test_mysql("SELECT 1");
        assert_eq!(handler.output_variable(&mysql), None);
    }
    ...
    async fn test_variable_substitution() {
        ...
        let mysql = create_test_mysql(
            "CREATE TABLE {{ var.table_name }} (id INT, name VARCHAR(255)); \
        ...
        let handler = MySQLHandler;
        let handle = handler
            .execute(mysql, context, _tx, None)
            .await
            .expect("MySQL execution should start");
        ...
        loop {
            ...
            match status {
                ...
                ExecutionStatus::Cancelled => panic!("MySQL query was cancelled"),
    ...
    async fn test_empty_uri() {
        ...
        let mut mysql = create_test_mysql("SELECT 1");
        mysql.uri = "".to_string();

        let handler = MySQLHandler;
        let handle = handler
            .execute(mysql, create_test_context(), _tx, None)
            .await
            .expect("MySQL execution should start");
    ...
    async fn test_cancellation() {
        ...
        let mysql = create_test_mysql("SELECT SLEEP(10)");
    ...
    async fn test_grand_central_events_failed_query() {
        ...
        // Create MySQL query that will fail
        let mysql = create_test_mysql("INVALID SQL");
        let mysql_id = mysql.id;

        let handler = MySQLHandler;
        let handle = handler.execute(mysql, context, tx, None).await.unwrap();
